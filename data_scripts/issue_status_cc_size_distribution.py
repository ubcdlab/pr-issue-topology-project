...
def main():
    ...
    plt.bar(
        ...
        list(
            map(
                lambda x: x[0] / x[1],
                list(map(list, zip(closed_map.values(), total_map.values()))),
            )
        ),
        color="#FAA0A0",
        label="Closed Issues",
    ...
    plt.bar(
        ...
        list(
            map(
                lambda x: x[0] / x[1],
                list(map(list, zip(open_map.values(), total_map.values()))),
            )
        ),
        bottom=list(
            map(
    ...
    try:
        ...
        pass
    plt.savefig(
        f"misc_images/issue_status_distribution_cc_size.png",
        bbox_inches="tight",
        dpi=150,
    )
