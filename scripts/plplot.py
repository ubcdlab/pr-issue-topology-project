...
def plplot(x, xmin, alpha, ks):
    ...
    if f_dattype == "REAL":
        ...
        cf = map(lambda X: pow(float(X) / xmin, 1.0 - alpha), q)
        ...
        c1 = q + [q[-1] + 1]
        c2 = map(lambda Z: 1.0 - Z, reduce(lambda X, Y: X + [Y + X[-1]], c, [0]))
        c2 = list(filter(lambda X: float(X) >= pow(10, -10.0), c2))
        c1 = c1[0 : len(c2)]
        cf = map(
            lambda X: pow(X, -alpha) / (float(zeta(alpha)) - sum(map(lambda Y: pow(Y, -alpha), range(1, int(xmin))))),
            range(int(xmin), q[-1] + 1),
        )
