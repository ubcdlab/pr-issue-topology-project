...
def main():
    ...
    if not isfile("cc_size_pr_distribution.pickle"):
        with Pool(cpu_count() // 2) as p:
            with tqdm(total=num_graphs(), leave=False) as pbar:
                for res in p.imap_unordered(parallelize_graph_processing, all_graphs()):
                    ...
                    pbar.update()
        with open("cc_size_pr_distribution.pickle", "wb") as x:
            dump([open_map, closed_map, merged_map, total_map], x)
    else:
        with open("cc_size_pr_distribution.pickle", "rb") as x:
            open_map, closed_map, merged_map, total_map = load(x)
