...
with i, size(collect([i_node in nodes where i_node.type="issue" and i_node.number <> i.number and not id(i_node) in known_dups])) as not_dups, size(nodes) as len_nodes, nodes
return i, toFloat(not_dups) / toFloat(len_nodes) as proportion, nodes
