...
where (hub:pull_request or (hub:issue and hub.status = "closed")) and pr2.creation_date < hub.creation_date and not id(hub) in all_ids and not id(pr2) in all_ids
with hub, collect(distinct pr2) as prs, collect(distinct pr2.user) as users, collect(distinct r) as match_relationships, collect(distinct id(hub)) as known_hubs
...
with hub, prs, nodes, relationships, match_relationships, size(collect([i_node in nodes where (i_node.type="pull_request" and hub:pull_request) or (i_node.type="issue" and hub:issue) and i_node.number <> hub.number and not id(i_node) in known_hubs])) as not_prhub, size(nodes) as len_nodes
