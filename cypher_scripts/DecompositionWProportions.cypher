...
with i, size(collect([i_node in nodes where i_node.type="issue" and i_node.status="closed" and i_node.number <> i.number and not id(i_node) in known_decomposition])) as not_decomp, size(nodes) as len_nodes, nodes
return i, toFloat(not_decomp) / toFloat(len_nodes) as proportion, nodes
