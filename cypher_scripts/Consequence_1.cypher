...
return collect(distinct id(i))+collect(distinct id(p))+collect(distinct id(i2))+collect(distinct id(p2)) as known_consq_2
}
...
with known_hubs+known_consq_2 as exclude_ids , known_consq
match (i:issue {status:"closed"})-[r {labels: "fixes"}]-(p:pull_request {status: "merged"}), (i2:issue)-[r2]-(p)
where i2.creation_date > p.creation_date and i.number <> i2.number and not id(i) in exclude_ids and not id(p) in exclude_ids and not id(i2) in exclude_ids
