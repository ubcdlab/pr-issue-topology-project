...
return collect(distinct id(i)) as known_extended
}

match (pr1:pull_request)-[r {labels:"fixes"}]-(i:issue),(pr1)-[r2]-(pr2:pull_request),(pr2)-[r3 {labels:"fixes"}]-(i)
where not id(i) in all_ids and not id(pr1) in all_ids and not id(pr2) in all_ids and pr1.number<>pr2.number and i.status ="closed" and pr1.status="merged" and pr2.status="merged" and pr2.creation_date > pr1.creation_date
with i, pr1, pr2, [r,r2,r3] as match_relationships, known_extended
call apoc.path.subgraphAll(i, {limit: 50, bfs:true})
yield nodes, relationships
with i, pr1, pr2, nodes, relationships, match_relationships, size(collect([i_node in nodes where i_node.type="issue" and i_node.status="closed" and i_node.number <> i.number and not id(i_node) in known_extended])) as not_ext, size(nodes) as len_nodes
return i, pr1, pr2, nodes, relationships, match_relationships, toFloat(not_ext) / toFloat(len_nodes) as proportion
