//Competing PRs
call {
match (i:issue {status: "closed"})-[r {labels:"fixes"}]-(pr:pull_request)
...
return collect(distinct id(i)) as known_competition
}

match (i:issue {status: "closed"})-[r {labels:"fixes"}]-(pr:pull_request)
with i, collect(distinct pr) as pull_requests, collect (distinct pr.user) as users, collect(r) as match_relationships, max(pr.creation_date) as max_date, min(pr.creation_date) as min_date, known_competition
where size([p_r in pull_requests where p_r.status="merged"]) = 1 and size([p_r in pull_requests where p_r.status="closed"]) >= 1 and size(pull_requests) >= 2 and size(users) > 1 and max_date - min_date <= 604800 // dates are in Unix timestamps so difference is in seconds
