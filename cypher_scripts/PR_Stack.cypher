...
match (pr:pull_request)-[r]->(pr_2:pull_request)-[r2]->(pr_3:pull_request)
unwind [pr.number, pr_2.number, pr_3.number] as number_list
with pr, pr_2, pr_3, collect(distinct number_list) as number_list, [r,r2] as match_relationships 
where size(number_list) = 3 and pr.creation_date < pr_2.creation_date < pr_3.creation_date and not (:pull_request)-->(pr) and not (pr_3)-->(:pull_request) and pr.status = "merged" and pr_2.status = "merged" and pr_3.status = "merged"
call apoc.path.subgraphAll(pr, {limit: 50, bfs: true })
yield nodes, relationships
return pr, pr_2, pr_3, nodes, relationships, match_relationships
