...
with hub, collect(distinct pr2) as prs, collect(distinct pr2.user) as users, collect(distinct r) as match_relationships, collect(distinct id(hub)) as known_hubs,collect(distinct id(pr2)) as known_hub_prs
where size(prs) >= 3 and size(users) >= 2
return known_hubs+known_hub_prs as all_ids
}
