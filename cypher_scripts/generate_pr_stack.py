...
def gen_cypher(size: int, subgraph=False):
    ...
    query += "\n"
    query += "where " + " < ".join(["pr.creation_date"] + [f"pr_{i}.creation_date" for i in range(2, size + 1)]) + "\n"
    ...
    query += (
        "where optional_issue = null or ("
        + " and ".join([f"not (optional_issue)--(pr_{i})" for i in range(2, size + 1)])
        + ")\n"
    )
    query += (
        "with pr, "
    ...
    query += (
        ...
        + f'not (:pull_request)-->(pr) and not (pr_{size})-->(:pull_request) and size([i in status_list where i = "merged"]) >= {size}/2 and max_users.value >= {size}/2\n'
    )
    if subgraph:
