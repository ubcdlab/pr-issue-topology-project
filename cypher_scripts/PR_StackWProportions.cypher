...
with pr, size(collect([i_node in nodes where i_node.type="pull_request" and i_node.number <> pr.number and not id(i_node) in known_stacks])) as not_stack, size(nodes) as len_nodes
return pr, toFloat(not_stack) / toFloat(len_nodes) as proportion
